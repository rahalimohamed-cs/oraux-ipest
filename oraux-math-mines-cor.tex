...
\noindent $\bullet$ Soit $v \in \ker(\varphi)$. On a donc $v_{|\ker(p)} = 0$ soit $v(x) = 0 \quad \forall x \in \ker(p)$. Il suffit donc de montrer que $v(x) = 0 \quad \forall x \in \ker(p - \mathrm{id}_E)$, sachant que $p$ étant un projecteur, on a $E = \ker(p) \oplus \ker(p - \mathrm{id}_E)$.
...
\noindent $\bullet$ On a donc par le théorème du rang : $\dim(V) = \dim(\operatorname{Im}(\varphi))$. On peut tenter donc l'inégalité qui en résulte : $\dim(V) \leqslant \dim(\mathcal{L}(\ker p, E))$, mais par le théorème du rang appliqué à $p$, on a $\mathcal{L}(\ker p, E)$ de dimension $\dim(E) \cdot \dim(\ker p) = n(n-r) \geqslant (n-r)^2$. La borne n'est pas suffisamment restreinte.
...
\noindent On est en même de montrer que $U$ ou $V$ est égal à $\{0\}$. On a $n^2 \leqslant \dim(U) + \dim(V) \leqslant r^2 + (n-r)^2$, or on a aussi donc $n^2 - r^2 \leqslant (n-r)^2 \Leftrightarrow (n-r)(2r) \leqslant 0$. On déduit de cette inégalité que soit $r=0$, soit $n-r=s=0$, donc :
\begin{center}
